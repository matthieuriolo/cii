...
(function(rivets, moment) {
	...
	rivets.formatters.shiftAdd = function(target, val) {
		return val + target
	}
	...
	*/
	...
	/* number functions */
	rivets.formatters.shortNumber = function(target) {
		var suffix = '';
		if(target > 999) {
			suffix = 'k'
			target = target / 1000
		...
		if(target > 999 && suffix == 'B') {
			return '&infin;'
		}

		return rivets.formatters.number(target, 0) + suffix;
	}

	rivets.formatters.number = function(target, precision, decimalSeparator, thousandSeparator) {
	...
	rivets.formatters.date = function(target) {
		return moment(target).add(rivets.stdlib.defaultTimezoneSeconds, 'seconds').format(rivets.stdlib.defaultDateFormat)
	}

	rivets.formatters.time = function(target) {
		return moment(target).add(rivets.stdlib.defaultTimezoneSeconds, 'seconds').format(rivets.stdlib.defaultTimeFormat)
	}

	rivets.formatters.datetime = function(target) {
		return moment(target).add(rivets.stdlib.defaultTimezoneSeconds, 'seconds').format(rivets.stdlib.defaultDatetimeFormat)
	}

	rivets.formatters.toTimestamp = function(target) {
		return moment(target).add(rivets.stdlib.defaultTimezoneSeconds, 'seconds').format('X')
	}
	...
	rivets.formatters.toMoment = function(target) {
		return moment(target).tz(rivets.stdlib.defaultTimezone)
	}

	rivets.formatters.dateFormat = function(target, val) {
		return moment(target).add(rivets.stdlib.defaultTimezoneSeconds, 'seconds').format(val)
	}
	...
	/*
	some nice to have formatters
	*/
	
	rivets.formatters.htmlLink = function(target, show, def) {
		if(show) {
			if(target && rivets.formatters.isString(target)) {
				return '<a target="_blank" href="' + target + '">' + target + '</a>'
			}
			if(!def)
				def = '-'
			return def
	...
	rivets.formatters.htmlMailtoLink = function(target, show, def) {
		if(show) {
			if(target && rivets.formatters.isString(target)) {
	...
	rivets.formatters.htmlPhonetoLink = function(target, show, def) {
		if(show) {
			if(target && rivets.formatters.isInteger(target)) {
	...
	rivets.formatters.htmlGoogleMapsLink = function(target, country, show, def) {
		if(show) {
			var locName = []
			var loc = []

			if(target && rivets.formatters.isString(target)) {
				 locName.push(rivets.formatters.capitalize(target))
				 loc.push(target)
			}

			if(country && rivets.formatters.isString(country)) {
				locName.push(rivets.formatters.capitalize(country))
				loc.push(country)
			}
			if(!def)
				def = '-'
			if(!loc.length)
				return def
			return '<a target="_blank" href="https://www.google.ch/maps/place/' + encodeURIComponent(loc.join(',')) + '">' + locName.join(', ') + '</a>'
		}
		return ''
	};
	...
	rivets.binders.visible = function(el, value) {
...
})(rivets, this['moment'] ? moment : null)